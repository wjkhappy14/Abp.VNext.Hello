...
public class HelloHttpApiHostModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddHealthChecks().AddCheck("abp.vnext.hello.httpapi.host", () => HealthCheckResult.Healthy());
    }

    private void ConfigureCache(IConfiguration configuration)
    {
        Configure<AbpDistributedCacheOptions>(options => { options.KeyPrefix = "Hello:"; });
    }
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (!hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<HelloDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Abp.VNext.Hello.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<HelloDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Abp.VNext.Hello.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<HelloApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Abp.VNext.Hello.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<HelloApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Abp.VNext.Hello.Application"));
            });
        }
    }
